...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.id != undefined) {
      ...
      eventChannel.on('articleData', function (data) {
        that.setData({
          article: data,
        })
        that.getCommentList();
        app.wxparse.wxParse('articleHtml', 'html', data.content, that, 5);
      })
    }
    if (options.videoPlay != undefined){
      that.setData({
    ...
    let obj = {
      ...
      success: (res) => {
        if (res.S == 1) {
          ...
          app.common.addUserHistory({ type: 2, recordId: res.V.id })
          app.common.addUserTaskRecord({id:res.V.id,type:2,subtype:3});
          that.getCommentList();
        }else{
          that.setData({
            noData:true,
  ...
  pushCommentList: function () {
    ...
    let obj = {
      ...
      success: (res) => {
        if (res.S == 1) {
          that.setData({
            ['commentList']: that.data.commentList.concat(res.V),
  ...
  onShareAppMessage: function () {
    var that = this;
    app.common.addUserTaskRecord({id: that.data.article.id , subtype: 6 })
    app.common.addShareCount({id:that.data.article.id});
    return {
      title: that.data.article.title,
      path: "/pages/community/article/page?id=" + that.data.article.id + '&videoPlay=' + that.data.videoPlay,
